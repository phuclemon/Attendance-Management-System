...
class Subject:
    def __init__(self,root):
        ...
        # nclass
        nameLabel = Label(left_inside_frame, text="Lớp:", font=("times new roman", 12, "bold"),
                          bg="white")
        ...
        search_combo = ttk.Combobox(Right_frame, font=("times new roman", 13, "bold"), textvariable=self.var_com_search,
                                    state="read only",
                                    width=13)
        search_combo["values"] = ("ID Môn học", "Tên môn học", "Lớp")
        ...
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                ...
                if(self.var_com_search.get()=="ID Môn học"):
                    ...
                    if(self.var_com_search.get()=="Lớp"):
        ...
        if self.var_com_searchtc.get()=="" or self.var_searchtc.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='',
                                               database='face_recognizer', port='3306')
                my_cursor = conn.cursor()#"ID Điểm Danh", "Ngày", "ID Học sinh "
                if(self.var_com_searchtc.get()=="ID Học sinh"):
